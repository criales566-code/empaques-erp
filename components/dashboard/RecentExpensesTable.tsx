...
export function RecentExpensesTable({ expenses }: { expenses: Expense[] }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      {/* Header */}
      <div className="flex items-center justify-between gap-3 px-7 sm:px-8 py-6 border-b border-slate-100">
      ...
      {expenses.length === 0 ? (
        ...
        <div>
          {expenses.map((expense) => (
            <div
              key={expense.id}
              className="flex items-center gap-3 sm:gap-4 px-7 sm:px-8 py-5 border-b border-slate-50 last:border-0 hover:bg-slate-50/60 transition-colors"
            >
